...
func main() {
    ...
    fmt.Println(strings.Trim("sqnssfewfewsqn", "sqn")) //在s字符串的头部和尾部去除cutset指定的字符串
    fmt.Println(strings.Join(strings.Fields("a b sqn hello world sqn"), "--")) //去除s字符串的空格符，并且按照空格分割返回slice,并连接
    /*字符串转化的函数在strconv中*/
    //Append 系列函数将整数等转换为字符串后，添加到现有的字节数组中。
    str := make([]byte, 0, 100)
    str = strconv.AppendInt(str, 44, 10) //10进制
    str = strconv.AppendInt(str, 5, 2) //2进制
    str = strconv.AppendBool(str, false)
    str = strconv.AppendQuote(str, "abcd")
    str = strconv.AppendQuoteRune(str, '你')
    str = strconv.AppendQuoteRuneToASCII(str, '好')
