...
class Paddle:
    ...
    def move(self, direction):
        ...
        """
        # 计算新位置
        new_y = self.rect.y + direction * PADDLE_SPEED
        
        # 限制在屏幕内
        if new_y < 0:
            new_y = 0
        if new_y > SCREEN_HEIGHT - PADDLE_HEIGHT:
...
class PingPongGame:
    ...
    def quick_train_episode(self):
        ...
        while player1.score < 11 and player2.score < 11:
            ...
            if ball.rect.left <= 0:
                ...
                ball.reset(1)
                # 重置连续击球计数
                self.rl_agent1.consecutive_hits = 0
                self.rl_agent2.consecutive_hits = 0
    ...
    def handle_menu(self):
        ...
        # 标题 - 使用get_rect替代方法
        title_surface = self.font.render("乒乓球游戏 - 强化学习训练", True, WHITE)
        ...
        if self.training_phase == "pre_train":
            ...
            progress_surface = self.small_font.render(progress, True, YELLOW)
            progress_rect = progress_surface.get_rect(center=(SCREEN_WIDTH//2, 100))
            self.screen.blit(progress_surface, progress_rect)
        elif self.training_phase == "ready":
            ready_text = "预训练完成！AI已准备好！"
            ready_surface = self.small_font.render(ready_text, True, GREEN)
            ready_rect = ready_surface.get_rect(center=(SCREEN_WIDTH//2, 100))
        ...
        for i, option in enumerate(options):
            ...
            self.screen.blit(option_surface, (SCREEN_WIDTH//2 - 200, 150 + i * 30))
        ...
        if self.rl_agent1.total_games > 0:
            ...
            for i, stat in enumerate(stats):
                stat_surface = self.small_font.render(stat, True, GREEN)
                self.screen.blit(stat_surface, (SCREEN_WIDTH//2 - 200, 350 + i * 25))
    ...
    def draw(self):
        ...
        p1_surface = self.small_font.render(p1_text, True, BLUE)
        p2_surface = self.small_font.render(p2_text, True, RED)
        
        self.screen.blit(p1_surface, (50, 20))
        self.screen.blit(p2_surface, (SCREEN_WIDTH - 150, 20))
    ...
    def load_ai_models(self):
        ...
        try:
            ...
            print(f"加载AI1模型失败: {e}")
            self.rl_agent1 = RLAgent(1)  # 重新初始化
        
        try:
            self.rl_agent2.load_model("agent2_model.pkl")
        except Exception as e:
            print(f"加载AI2模型失败: {e}")
            self.rl_agent2 = RLAgent(2)  # 重新初始化
